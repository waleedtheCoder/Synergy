...
const NAV_ITEMS = [
  { href: ROUTES.professionalDashboard, label: "Overview", icon: LayoutGrid },
  { href: ROUTES.proDashboardProfile, label: "Profile", icon: UserRound },
  ...
  { href: ROUTES.proDashboardSettings, label: "Settings", icon: Settings },
];
      ...
      {NAV_ITEMS.map((item) => {
        ...
        return (
          ...
          >
            ...
            {item.href === ROUTES.proDashboardNotifications && unreadCount > 0 && (
...
export function ProDashboardShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!isHydrated || !user || user.role !== "PROFESSIONAL") return;
  ...
  if (!isHydrated || !user || user.role !== "PROFESSIONAL") {
    return (
